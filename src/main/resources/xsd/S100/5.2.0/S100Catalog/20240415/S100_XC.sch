...
<!--
  ...
  © Copyright 2018, 2022, 2023, 2024 (IHO)
  ...
  4.0.0-20180502  Updated for Edition 4.0.0 and ISO 19115-3 (Raphael Malyankar, Portolan Sciences LLC)
  ...
  5.0.0-20220331  Updated for S-100 Edition 5.0.0 (Raphael Malyankar, Portolan Sciences LLC)
        Rules updated, removed, and added to implement constraints in new Part 17; unneeded namespaces removed
        20220620  New rule to check for presence of otherLocale referenced in PT_FreeText elements
        20220705  Removed obsolete rules and separated unused prefixes
        20230105  Version updated to be consistent with the other XSDs in same folder
  5.1.0-20230327  Updated for S-100 Edition 5.1.0 (Raphael Malyankar, Portolan Sciences LLC)
        Namespaces for S-100 exchange catalogue and signature/encryption updated; added check for
        bounding polygon in dataCoverage. Note that S-100 does not say it cannot have holes.
  5.2.0-20231214  Rule checking for DSA signature replaced with check for ECDSA-384-SHA2 because S-100 5.2 changed the
                  required signature algorithm; rules checking for use of UTC where UTC is specified in Part 17 Remarks;
                  note that the schemaVersion is for this file, the schemaVersion of the exchange catalogue schema remains unchanged
                  because all the Part 15 algorithms are already encoded
        20240124  Rule to check that bounding polygons use LinearRing for exterior and interior; check id
        20240415  Namespace updates for Ed. 5.2.0 - exchange catalogue and Part15 xsds have updated namespaces
                  
	-->
<!-- ============================================================================================= -->
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" schemaVersion="5.2.0-20231214">
  ...
  <sch:ns uri="http://www.iho.int/s100/xc/5.2" prefix="S100XC"/>
  <sch:ns uri="http://www.iho.int/s100/se/5.2" prefix="S100SE"/>
...
<!--
  Rule to validate relative values of max and minimum display scales.
  Modified from 4.0 because Ed. 5.0.0 removed maximum and minimum display scales from dataset discovery metadata
-->

  <sch:pattern id="S100_DC_MaxMin" fpi="S100_17.DataCoverage">
...
<!-- 
  Rule to require that bounding polygon consist of gml polygon
-->
  <sch:pattern id="S100_DC_BoundingPolygon" fpi="S100_17_DataCoverage_boundingPolygon">
    <sch:title>Check that bounding polygon is a gml:Polygon</sch:title>
    <sch:rule context="//S100XC:dataCoverage/S100XC:boundingPolygon" role="warning">
      <sch:assert test="gex:polygon/gml:Polygon[normalize-space(gml:exterior) != '']">
        <sch:value-of select="../../S100XC:fileName"/> dataCoverage bounding polygon must be a gml:Polygon
      </sch:assert>
    </sch:rule>
 </sch:pattern>
  
<!-- 
  Rule to require that bounding polygon consist of gml polygon with LinearRing elements
  Constraint introduced in S-100 5.2.0
-->
  <sch:pattern id="S100_DC_BoundingPolygonLinearRing" fpi="S100_17_DataCoverage_boundingPolygonLinearRing">
    <sch:title>Check that bounding polygon is a gml:Polygon with LinearRing used for exterior and interior</sch:title>
    <sch:rule context="//S100XC:dataCoverage/S100XC:boundingPolygon//gml:exterior|//S100XC:dataCoverage/S100XC:boundingPolygon//gml:interior" role="warning">
      <sch:assert test="gml:LinearRing">
        dataCoverage bounding polygon must be a gml:Polygon whose <sch:value-of select="local-name()"/> is a LinearRing
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern id="S100_NameEmpty" fpi="S100_XC_cString_generic_productSpecification">
  ...
  <sch:pattern id="S100_RolePresentOrNil" fpi="S100_XC_DSMD_role_Nillable">
  ...
  -->
  <sch:pattern id="S100_FilenamePopulated" fpi="S100_XC_fileName">
  ...
  -->
  <sch:pattern id="S100_PSNamePopulated" fpi="S100_XC_productSpecification">
  ...
  <sch:pattern id="S100_ECDSA384Signature" fpi="S100.5.2_15-8.7">
    <sch:rule context="//S100XC:digitalSignatureReference" role="warning">
      <sch:report test="not(text() = 'ECDSA-384-SHA2')">
        <sch:name/> (<sch:value-of select="local-name(..)"/>) <sch:value-of select="."/> must be ECDSA-384-SHA2 in S-100 5.0 catalogs
      </sch:report>
    </sch:rule>
  </sch:pattern>
  ...
  <!-- Validate temporal extent by presence of at least one sub-attribute. In 5.0, S100_TemporalExtent is used only in dataset discovery metadata,
        either locally in attribute temporalExtent or in a sub-element of the dataCoverage attribute -->
  <sch:pattern id="S100_TemporalExtentValid" fpi="S100_TemporalExtent">
    ...
    <!-- check for temporal extent in datacoverage is the same except for the context, but is given separately for readability -->
    <sch:rule context="/S100XC:S100_ExchangeCatalogue/S100XC:datasetDiscoveryMetadata/S100XC:S100_DatasetDiscoveryMetadata/S100XC:dataCoverage/S100XC:temporalExtent">
      <sch:assert test="count(S100XC:timeInstantBegin|S100XC:timeInstantEnd) > 0">
        <sch:name/>: At least one of timeInstantBegin/timeInstantEnd must be populated if temporalExtent is present
      </sch:assert>
    </sch:rule>
  ...
  <sch:pattern id="S100_CataloguePurposeValid" fpi="S100_CatalogueDiscoveryMetadata.purpose">
  ...
  <!-- Rule to chack that locale used in PT_FreeText is defined. otherLocale for other languages used in the catalogue must be in the catalog header -->
  <sch:pattern id="S100_LocaleDefined" fpi="S100_PT_FreeText_Locale_Defined">
    <sch:title>Check for existence of locale used in PT_FreeText</sch:title>
    <sch:rule context="//lan:PT_FreeText/lan:textGroup/lan:LocalisedCharacterString" role="warning">
      <sch:let name="LOCALE_ID" value="substring-after(@locale, '#')"/>
      <sch:assert test="count(/S100XC:S100_ExchangeCatalogue/S100XC:otherLocale/lan:PT_Locale[@id = $LOCALE_ID])">
        No locale defined for <sch:value-of select="$LOCALE_ID"/> in the exchange catalogue header
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  
  <!-- Check that date time is UTC in format specified in Part 17 -->
  <sch:pattern id="S100_DTGFormat" fpi="S100_DTGZ">
    <sch:title>Check format of dateTime for conformance to S-100 Part 17 constraints</sch:title>
    <sch:rule context="/S100XC:S100_ExchangeCatalogue/S100XC:identifier/S100XC:dateTime" role="warning">
      <sch:let name="DTG" value="string()"/>
      <!-- using only XPath 1 functions in test -->
      <sch:assert test="(string-length($DTG) = 20) and (substring($DTG, 20) = 'Z')">
        ExchangeCatalogueIdentifier/identifier is <sch:value-of select="$DTG"/>, must be in format YYYY-MM-DDTHH:MM:SSZ
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  
  <!--
    Rule to check that certificate issuers are listed in the file as SA or domain coordinator
    Ref. S-100 5.2.0 15-8.6
    Note: This rule also emits a warning if id and issuer are the same
  -->
  <sch:pattern id="S100_CertIssuerIDCompleteChain" fpi="S100_5_2_0_P15_8_6">
    <sch:title>Check that certificate issuer id is ID of SA or another issuer listed in the exchange catalog</sch:title>
    <sch:rule context="/S100XC:S100_ExchangeCatalogue/S100XC:certificates/S100SE:certificate" role="warning">
      <sch:let name="ID" value="string(@id)"/>
      <sch:let name="ISSUER" value="string(@issuer)"/>
      <sch:let name="DOMAINADMINS" value="count(preceding-sibling::node()[@id = $ISSUER]) + count(following-sibling::node()[@id = $ISSUER])"/>
      <sch:assert test="($ISSUER = string(../S100SE:schemeAdministrator/@id)) or ($DOMAINADMINS &gt; 0)">
        Certificate element (id=<sch:value-of select="@id"/>, issuer=<sch:value-of select="@issuer"/>)issuer attribute of certificate element must match id of scheme administrator or certificate issuer
      </sch:assert>
    </sch:rule>
